...
```{r}
###create new varible/vector called "Lo" 
...
```{r}
###### Since algorithm will treat each row as a value belonging to a person, so remove the name column and remoce the "regions" columns
...
```{r}
###select question3 and transfer into numeric answers
...
```{r}
###select question4 and transfer into numeric answers
...
###Third part : now there are two groups of data: one group is  the "regions" data (K4), and the other is the "questions" (T2), the followings are trying to create visualization for those two groups of data
...
```{r}
###some interesting plots for T2 & K4
