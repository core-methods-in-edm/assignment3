...
#Each row represents a comment from one student to another so the first line shows that student "28" commented on the comment of student "21". It also shows the gender of both students and the students' main elective field of study ("major"").
```
...
Use [look up](http://igraph.org/r/) in the igraph documentation and modify the graph above so that:
...
Create a **person-network** with the data set hudk4050-classes.csv. To create this network create a person-class matrix using the tidyr functions and then create a person-person matrix using `t()`, then plot a matrix rather than a to/from data frame using igraph.

Also [look up](http://igraph.org/r/) how to generate the following network metrics:
