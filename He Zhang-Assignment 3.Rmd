...
```{r}
K1 <- read.csv("~/Desktop/2017 Fall/EDM/He Zhang-Assignment 3/Class_Motivation.csv")
```
...
#PART II
```{r}
DF1 <- read.csv("~/Desktop/He Zhang-Assignment 3/cluster-class-data.csv")
...
```{r}
library(dplyr)
D1<-apply(D1, 2, function(x){x[x=="Yes"]<-1;x})
D1<-apply(D1, 2, function(x){x[x=="No"]<-0;x})
D1<-apply(D1, 2, function(x){x[x=="Did not answer"]<-0;x})
```
