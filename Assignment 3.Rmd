...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, vertex.size=a*2,edge.arrow.size=0.3,margin=-0.1)
```
## Part III
...
#standardize classname format & ignore NA/empty/HUDK4050 classes
...
```{r}
#person-person matrix
graph_pp <- graph_from_adjacency_matrix(person_person, mode = c("undirected"), weighted = T,
  diag = FALSE)
#highest betweenness centrality
pp_betweeness <- betweenness(graph_pp)
pp_betweeness[which.max(pp_betweeness)]
#highest degree centrality
pp_degree <- degree(graph_pp)
pp_degree[which(pp_degree==max(pp_degree))]

#person-class matrix
graph_pc <- graph_from_incidence_matrix(person_class, directed = FALSE)
#highest betweenness centrality
pc_betweeness <- betweenness(graph_pc)
pc_betweeness[which.max(pc_betweeness)]
#highest degree centrality
pc_degree <- degree(graph_pc)
pc_degree[which(pc_degree==max(pc_degree))]
```
**Yifei zhang is the most central person in the network according to the two metrics, since he/she has both the highest betweeness centrality and dregree centrality.**
...
```{r}
#dataframe with interest
df3 <- select(df1,name,interest)
df3$interest <- as.factor(df3$interest)

#plot person-to-person graph
plot(graph_pp,layout=layout.fruchterman.reingold,vertex.label.cex=0.3,vertex.color=df3$interest)

#plot person-to-class graph
plot(graph_pc,layout=layout.fruchterman.reingold,edge.width=0.3,vertex.label.cex=0.3,vertex.color=df3$interest)
```
**There isn't any cluster of interest that correspond to cluster in the network according to the two pictures above. Interests are randomly distributed.** 
