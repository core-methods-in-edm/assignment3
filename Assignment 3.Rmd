...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=EDGE$count, vertex.size=30)

plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=EDGE$count, vertex.size=25, edge.arrow.size=0.3)

commentNum <- count(D2, comment.to)
names(commentNum) <- c("id", "count")
commentNum <- left_join(VERTEX,commentNum,by=c("id"))
commentNum$count[is.na(commentNum$count)] <- 0

plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=EDGE$count, vertex.size=15+commentNum$count, edge.arrow.size=0.5)
