...
n = num + 12
plot(g, layout = layout.fruchterman.reingold, vertex.color = VERTEX$major, edge.width = EDGE$count, edge.arrow.size = 0.6, edge.arrow.width = 0.6, vertex.size = n,vertex.label.cex = n/15, vertex.label.color = "black")
```
...
# Yifei Zhang is the most central person in the network according to those two metrics. 
# Betweenness centrality quantifies the number of times a node acts as a bridge along the shortest path between two other nodes; Since Yifei zhang has the highest Betweenness centrality and a relatively high dregree centrality, she is the most connected person in the network.
```
...
color <- as.factor(c5$Interest)
plot(g, layout = layout.fruchterman.reingold, vertex.color=as.factor(c5$Interest),
     vertex.size =10, vertex.label.cex = 0.8, edge.width = 1, vertex.label.color = "black")
# There is no obvious cluster of interest that corresponds to clusters in the network. It indicates that the interests of students are not related to classes they take.  
```
