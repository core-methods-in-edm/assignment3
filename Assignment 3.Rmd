...
library(igraph)
library(reshape2)
data<-read_csv("C:/Users/apple/Desktop/r projects/assignment2/hudk4050-classes.csv")
...
plot(graph1,layout=layout.fruchterman.reingold,vertex.color=fit2$cluster)
...
F1<-read.csv("HUDK405019-clustering.csv")
...
F5<-scale(F5)
fit4<-kmeans(F5,3)
F6<-data.frame(F5,fit4$cluster)

t1<-as.data.frame(fit4$cluster)
t2<-as.data.frame(fit2$cluster)
name <- rownames(t1)
rownames(t1) <- NULL
t1 <- cbind(name,t1)
name <- rownames(t2)
rownames(t2) <- NULL
t2 <- cbind(name,t2)
t3<-inner_join(t1,t2,by="name")
#install.packages("vcd")
library(grid)
library(vcd)
cluster1<-as.numeric(t3$`fit4$cluster`)
cluster2<-as.numeric(t3$`fit2$cluster`)
ov<-structable(cluster1~cluster2)
mosaic(ov,shade=TRUE,legend=TRUE)
