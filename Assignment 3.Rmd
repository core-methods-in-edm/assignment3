...
```{r}
plot(g,edge.arrow.size=.5,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, vertex.size=EDGE$count*5,edge.width=EDGE$count)
```
...
D1 <- read.csv("hudk4050-classes.csv", stringsAsFactors = FALSE, header = TRUE)
D2 <- D1

colnames(D2) <- D2[1,]
D2 <- slice(D2,3:49)
D2 <- select(D2,1:8)
D2 <- unite(D2, "name", 'First Name', 'Last Name', sep = " ")
D2$name <- str_replace(D2$name, "'", "")
D2$name <- str_to_title(D2$name)
D2 <- D2 %>% mutate_at(2:7, list(toupper))
D2 <- D2 %>% mutate_at(2:7, str_replace_all, " ", "")
```

```{r}
D3 <- D2 %>% gather(label, class, 2:7, na.rm = TRUE, convert = FALSE) %>% select(name, class)
D3$count <- 1
D3 <- filter(D3, class != "")
D3 <- unique(D3)
D3 <- spread(D3, class, count)
rownames(D3) <- D3$name
...
```{r}
g <- graph.adjacency(D4, mode="undirected", diag = FALSE)
