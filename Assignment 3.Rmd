...
#union is like "joint" for vector, joint only works for data frame
...
#ifelse doesn't work well with factor, so change it ot character first
...
plot(g,layout=layout.fruchterman.reingold)
plot(g,layout=layout.fruchterman.reingold,vertex.size=4,vertext.label.cex=0.8,vertex.label.color="black",vertex.color="gainsboro")
```
...
From the output, we can see that Yifei Zhang has the highest betweenness centrality, that means Yifei Zhang has the most shortest paths pass through, which is about 260. It's way more than the second one Si Heng Zhao. This result matches with the network graph above, Yifei Zhang is the connection point of two big clusters in the graph. If we wanted to connect the two big clusters, Yifei might be the breakthrough point. 
...
From the degree centrality analysis, we don't see one person has much more connections than others. Eight students have measure of degree centrality of 31, which means they are connected with 31 students in the class (taking the same course(s)) respectively, no one has particularly high degree of centrality, this matches with what we see in the graph as well. 

In this case, betweenness centrality tell us more information about the network than degree of centrality. If we wanted to connect the two big clusters, Yifei might be the breakthrough point. 
...
#specify the color graphical parameter
library(RColorBrewer)
coul  <- brewer.pal(9, "Set1")
V(g)$color <- coul[Ci$interest]
#graph
g<-graph.adjacency(C4,mode="undirected",diag=FALSE)
plot(g,layout=layout.fruchterman.reingold,
