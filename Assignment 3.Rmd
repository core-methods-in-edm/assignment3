...
```{r}
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, edge.width=EDGE$count, margin=-0.1, edge.arrow.size=0.35, edge.arrow.width=0.7,  vertex.size = (EDGE$count*10), vertex.label.color = "saddlebrown")
```
...
class <- read.csv("hudk4050-classes.csv", stringsAsFactors=F, header=T)
cls2 <- class
colnames(cls2) <- cls2[1,]
cls2 <- slice(cls2, 3:49)
cls2 <- select(cls2, 1:8)
cls2 <- unite(cls2, "name", `First Name`, `Last Name`, sep=" ")

cls2$name <- str_replace(cls2$name, "`", "")
cls2$name <- str_to_title(cls2$name)
cls2 <- cls2 %>% mutate_at(2:7, list(toupper))
cls2 <- cls2 %>% mutate_at(2:7, str_replace_all, " ", "")

cls3 <- cls2 %>% gather(label, class, 2:7, na.rm=T, convert=F) %>% select(name, class)
cls3$count <- 1
cls3 <- filter(cls3, class != "")
cls3 <- unique(cls3)
cls3 <- spread(cls3, class, count)
rownames(cls3) <- cls3$name
cls3 <- select(cls3, -name, -HUDK4050)
cls3[is.na(cls3)] <- 0
...
g <- graph.adjacency(cls4, mode="undirected", diag=F)
...
# Yifei Zhang is the most central person in the network according to these two metrics.
# Since he/she has highest degree centrality and very high level of betweenness centrality.
# There are several people has same level of degree centrality as Yifei, but their betweenness centrality are much less than Yifei.
...
cls5 <- class
cls5 <- slice(cls5, 3:49)
cls5 <- select(cls5, c(1,2,9))
cls5 <- unite(cls5, "name", Q8, Q9, sep=" ")
cls5$name <- str_replace(cls5$name, "`", "")
cls5$name <- str_to_title(cls5$name)
colnames(cls5) <- c("name", "interest")
color <- as.factor(cls5$interest)
plot(g, layout=layout.fruchterman.reingold,
     vertex.size=4,
     vertex.label.cex=0.8,
     vertex.label.color="black",
...
# The plot shows that interest are ramdomly distributed (the colors distribute randomly). So there is no clusters of interest that correspond to clusters in the network.
