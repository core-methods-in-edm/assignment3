...
If we recode the missing value, it will bias the result because it will treat the missing value the same. For example, if we recode the missing value as 0 then very missing value will be 0, in fact these values wouldn't be same.
If we omit these missing values, it will bias our result too because we will missing informations.
...
Cluster 1 shows people are getting more motivated towards the end
Cluster 2 shows people are losing motivation.
...
3 clusters is more informative because it shows another group of student who are more stable in terms of motivation. 
Cluster 3 shows students who have motivation at positive 1 value graduately lost their motivation to below zero.
CLuster 2 shows students are more stable in motivation level fluctuates from -0.5 to +0.5
Cluster 1 shows students who have negative motivation and graduately increase their motivation to positive value towards the end.
...
C1 <- read.csv("HUDK405018-cluster.csv")
...
C2[C2==""] <- NA
C2 <- C2 %>% mutate_all(funs(as.numeric(levels(.))[.]))

#Recode as some other number that doesn't appear in your data (EG - 99) and rescale
C2[is.na(C2)] <- 99
