...
for (i in 1:length(vertex_id)){
  new=length(which(vtoid==vertex_id[i]))
  num=c(n,new)
}
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, edge.arrow.size=0.4,vertex.size=num*2)
...
C2$name <- str_replace(C2$name, "`", "")
C2$name <- str_to_title(C2$name)
colnames(C5) [2]<- paste("Interest")
interest <- as.factor(C5$Interest)
plot(g, layout=layout.fruchterman.reingold, 
     vertex.size=8, 
