...
```{r}
plot(g,layout=layout.fruchterman.reingold,vertex.color=VERTEX$major, edge.arrow.size=.4, vertex.size=5)
...
```{r}
plot(g,layout=layout.fruchterman.reingold,vertex.color=VERTEX$major, edge.arrow.size=.3, vertex.size=VERTEX$major*5,vertex.label.color="black")
...
t <- read.csv("hudk4050-classes.csv", stringsAsFactors = FALSE,header = TRUE)
colnames(t) = t[1, ] 
t = t[-(1:2),]
names(t)[names(t) == "First Name"] <- "First_name"
t <- t %>% mutate_at(3:8, list(toupper))
t <- t %>% mutate_at(3:8, str_replace_all, " ", "")

#Data restructuring 
T2 = subset(t, select = c("First_name", "Class 1","Class 2","Class 3","Class 4","Class 5","Class 6"))
longT <- gather(T2, key ="class", value="course",-First_name)
longT = subset(longT, select = -c(class) )

T3 <- na.omit(longT)
T3$ count<- 1
T3<- filter (T3, course != "")
T3 <- unique(T3)
T3<- spread(T3,course, count)

rownames(T3) <- T3$First_name
T3<- select(T3, -First_name)
T3[is.na(T3)]<- 0

#Matrix creation, Person to class and Person to person 
T3<-as.matrix(T3)
T4 <- T3 %*% t(T3)

#Graph plot
G=graph_from_adjacency_matrix(T4,mode = c('undirected'),weighted = NULL, diag=F)
plot(G,edged.curve=0.3,vertex.size=7,vertex.color="darkgreen",edge.curved=0,vertex.label.cex=0.7)

```
* Betweeness centrality and dregree centrality. 
...
##Looking at two metrics, Yingxin and Yifei are the most central people in the class##
```
...
```{r}
T5 <- read.csv("hudk4050-classes.csv", stringsAsFactors = FALSE,header = TRUE)
T5<-T5
colnames(T5) <- T5[1,]
T5 <- slice(T5, 3:49)
names(T5)[names(T5) == "First Name"] <- "First_name"
names(T5)[9] <- "interest"
T6 <- select(T5, First_name, interest)
plot(G, layout=layout.fruchterman.reingold, vertex.size=10, edge.curved=0,vertex.label.cex =0.3, vertex.label.color="black", vertex.color=as.factor(T6$interest))
##There are no prominent clusters in the network##
