...
```{r}
v_size = EDGE %>% group_by(to) %>% summarise(count = sum(count))

names(v_size) = c('id', 'count')

VERTEX = left_join(VERTEX, v_size, by = 'id')
...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, edge.width=EDGE$count, edge.arrow.size = 0.7, vertex.size=(VERTEX$count+1)*3.5)
```
...
```{r}
#read the csv and skip the first line
C1 <- read.csv("hudk4050-classes.csv",skip = 1, header = TRUE, na.strings = "", stringsAsFactors = TRUE)
#make a copy of C1
C2 <- C1
#drop the title line
C2 <- slice(C2, 2:49)
...
```{r}
#turn it into person-class matrix
C4 <- as.matrix(C3)
#turn it into person-person matrix
C4 <- C4 %*% t(C4)
```

```{r}
#plot the person-person matrix
g <- graph.adjacency(C4, mode = "undirected", diag = FALSE)
plot(g, layout = layout.fruchterman.reingold,
     vertex.label.cex = 0.6,
     vertex.label.color = "black",
     vertex.size = (diag(C4)+1)*1.8)
```
...
From the graph and the metrics, I think Yifei Zhang is the most central person in the network
...
plot(g, layout = layout.fruchterman.reingold,
     vertex.label.cex = 0.6,
     vertex.label.color = "black",
     vertex.color = as.factor(as.character(C2[order(C2$name),8])),
     vertex.size = (diag(C4)+1)*1.8)
```
