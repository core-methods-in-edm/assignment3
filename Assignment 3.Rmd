...
Group one had high average motivation at first, the motivation decreased with week; Group two had low average motivation at first, but motivation increased with week
...
K10 <- K9%>%group_by(week, cluster)%>%summarize(avg=mean(motivation))
...
# I think grouping by 3 clusters is more informative.
```
...
```{r}
ClassData <- read.csv("HUDK405018-cluster.csv")
ClassData2 <- ClassData[3:47, 20:26]
ClassData2[ClassData2==""] <- NA
ClassData2 <- ClassData2 %>% mutate_all(funs(as.numeric(levels(.))[.]))
ClassData2 <- na.omit(ClassData2)

ClassData2 <- as.data.frame(scale(ClassData2))

#Generate clusters for survey questions
fit3 <- kmeans(ClassData2, 3)

#Create a dataframe that only includes location data
L1 <- ClassData[3:47, 18:19]
