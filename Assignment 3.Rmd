...
We should treat them as having meaning. When we recode the missing values, we we will have same dimension as given. 
...
K6$cluster <- as.factor(K6$ccluster)
```
...
There is a two lines run chart in the plot.

It would be useful to determine how many people are in each cluster. We can do this easily with dplyr.

```{r}
K7 <- dplyr::count(K4, cluster)
names(K7) <- c("#cluster", "#people")
