...
```{r}
received <- count(D2, comment.to)
names(received) <- c("id", "received")
VERTEX2 <- full_join(VERTEX, received, by = "id")
VERTEX2$received <- ifelse(is.na(VERTEX2$received) == TRUE, 0, VERTEX2$received)
...
plot(g,vertex.color=VERTEX2$major, edge.arrow.size =0.3,vertex.size = VERTEX2$received*3, vertex.label.dist = 0.1, edge.width=EDGE$count, rescale = F, layout = l*1.1)
...
DF1 <- DF
DF1 <- DF1 [-c(1,2),]
colnames(DF1) <- c("First_Name","Last_Name","Class1","Class2","Class3","Class4","Class5","Class6","Interest")

DF2 <- unite(DF1, "Full_Name", First_Name:Last_Name, sep = " ")

clean_up <- function(col) {
  # remove non-alpha-numerical characters
  t <- gsub("[^[:alnum:] ]", "", col)
  
  # remove multiple spaces with one only
  t <- gsub("\\s+", " ", t)
  return(toupper(t))
}

DF3 <- DF2 %>%
  mutate_at(1:8, clean_up)
...
DF5 <- gather(DF4,key,Class,2:7) %>%
  select(-key)

DF5$count <- 1

DF5 <- filter(DF5, Class != "")

# Remove duplicated rows
DF5 <- unique(DF5)


DF5 <- spread(DF5, Class, count)

rownames(DF5) <- DF5$Full_Name

DF5 <- select(DF5, -Full_Name, - HUDK4050)

DF5 <- ifelse(is.na(DF5), 0, 1)
...
```{r}
g <- graph.adjacency(DF5, mode = "undirected", diag = FALSE)

l <- layout_with_dh(g)
l <- norm_coords(l, ymin=-1, ymax=1, xmin=-1, xmax=1)
plot(g, layout = l*1, vertex.color= "blue", vertex.size = 1, vertex.label.dist = 0.8, edge.color = "red", rescale = F)
