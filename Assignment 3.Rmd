...
plot(graph2, layout=layout.fruchterman.reingold, edge.arrow.size=1 , vertex.color=D4$cluster)
...
```{r}
Class_gather_matrix$Name <- str_to_upper(Class_gather_matrix$Name) %>% str_remove_all("[[:space:]]")

Part2_gather_matrix<- data.frame(Class_gather_matrix,class_fit$cluster) %>% rename("Part3_cluster"=class_fit.cluster)

A2<-read.csv("HUDK405019-clustering.csv")

A3 <- str_to_upper(A2) %>% str_remove_all("[[:space:]]")
