...
```{r}
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$gender, edge.width=EDGE$count, edge.arrow.width = 0.01)
```
...
```{r}
library(dplyr)
plot(g,layout=layout.fruchterman.reingold, vertex.size =EDGE$count*10,vertex.color=VERTEX$id,edge.arrow.width = 0.01)
```
...
```{r}
library(tidyverse)
class_dt <- read_csv("hudk4050-classes.csv", skip = 1)
class_dt1 <- class_dt[-1,]
class_dt1 <- class_dt1 %>% mutate_at(3:8, list(toupper))
class_dt1 <- class_dt1 %>% mutate_at(3:8,str_replace_all," ","")


class_dt1$`Class 1` <-str_replace(class_dt1$`Class 1`, "HUDK4050", " ")
class_dt1$`Class 2` <-str_replace(class_dt1$`Class 2`, "HUDK4050", " ")
class_dt1$`Class 3` <-str_replace(class_dt1$`Class 3`, "HUDK4050", " ")
class_dt1$`Class 4` <-str_replace(class_dt1$`Class 4`, "HUDK4050", " ")
class_dt1$`Class 5` <-str_replace(class_dt1$`Class 5`, "HUDK4050", " ")

class_dt1[class_dt1==" "]<-NA
class_dt1$student <- paste(class_dt1$`First Name`,class_dt1$`Last Name`,sep=" ")

class_dt2 <- class_dt1 %>% gather(`First Name`,`Last Name`,`Class 1`,`Class 2`,`Class 3`,`Class 4`,`Class 5`,`Class 6`,) %>% select (`student`,`Last Name`)
names(class_dt2) <- c("student","class_id")
class_dt2 <- na.omit(class_dt2)

class_dt2$count <- 1
class_dt2_spread <- spread(class_dt2,class_id,count)
class_dt2_spread[is.na(class_dt2_spread)] <- 0
student_m <- select(class_dt2_spread,student)

student_m <- as.matrix(student_m)


class_dt2_spread<- class_dt2_spread[,-1]
class_dt2_spread <-as.matrix(class_dt2_spread)
rownames(class_dt2_spread) <- student_m

class_student<- class_dt2_spread
...
```{r}
class_class <- class_student%*% t(class_student)
class_class
```
...
```{r}
adjacent_list <- graph_from_adjacency_matrix(class_class,mode = "undirected",diag=FALSE)
plot(adjacent_list, layout=layout.fruchterman.reingold,
     vertex.size=4,
...
* Centrality
```{r}
sort(degree(adjacent_list),decreasing = TRUE)
sort(betweenness(adjacent_list),decreasing = TRUE)
```

* Interpretaion\
The most central person in the network is Guoliang Xu, Hangshi Jin, and those who have 31 as the degree centrality, and Yifei Zhang is the most central person according to these two matrics. These people have the higest commonality. Also, there are clusters in the network. Most of the students are taking the same class. It is probably because students are in same major and the courses they are taking are the major requirments.
