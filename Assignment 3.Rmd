...
```{r}
plot(g,layout=layout.fruchterman.reingold, edge.arrow.size=.3, vertex.size=EDGE$count * 4, vertex.color=VERTEX$major, edge.width=EDGE$count)
```
...
```{r}
dt <- data.frame(Betweeness = betweenness(g), drgree = degree(g))
dt <- dt[order(dt$Betweeness, decreasing = TRUE), ]
dt
#according to the metrics, Yifei Zhang is the most central person in the network.
```
...
```{r}
V(g)$interest <- factor(as.character(C1[,9]))
plot(g,layout=layout.fruchterman.reingold, vertex.color = V(g)$interest,
     vertex.size = 7, vertex.label.color="black", vertex.label.cex=.7)
```
