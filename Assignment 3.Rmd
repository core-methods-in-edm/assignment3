...
K2 <- K1[,-1]
#taking all columns and removing the first column. 
...
K5 <- gather(K4, Week, Motivation_Score, "1","2","3","4","5")
```

Now lets use dplyr to average our motivation values by week and by cluster.

```{r}
#group by, summarize, and average 
K5.5 <- group_by(K5, Week, Cluster)

K6 <-summarise(K5.5,mean=mean(Motivation_Score))
```
...
```{r}
ggplot(K6, aes (x=week, y=mean, group=Cluster))+
geom_line()
...
```{r}
#From the graph, you can see that week 1 and 3 display higher motivation. In comparison, week 2, 4, and 5 have low rate of motivation. 
```
...
```{r}
#Grouping by 2 clusters is significantly more informative than grouping by 3. It is visually pleasing and the differences are easily identifiable. 
```
...
classdata2 <- classdata%>%gather(classtype, coursename, Class.1, Class.2, Class.3, Class.4, Class.5, Class.6) %>% dplyr::select(-c(classtype)) %>% filter(First.Name != "ZIMO")

#dylyr:: calling dyplr for this line. 
...
DF1 <- read.csv("HUDK405019-clustering.csv")
