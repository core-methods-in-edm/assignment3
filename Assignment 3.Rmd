...
D2 <- select(D1, comment.to, comment.from) #select() chooses the columnsh
head(D2,5)
```
...
VERTEX$major.from <- ifelse(is.na(VERTEX$major.from) == TRUE, as.character(VERTEX$major.to), as.character(VERTEX$major.from))
...
#EDGE<-EDGE%>%
#    rename(from=from,to=to,weight=count)
g <- graph.data.frame(EDGE, directed=TRUE, vertices=VERTEX)

#set degree to adjust the size of the vertices
deg<-degree(g,mode = 'all')

#set the color
color_by_major <- c("red","pink","yellow","green")
V(g)$color<-color_by_major[factor(V(g)$major)]

plot(g,vertex.size=3*deg,vertex.label.dist=0,edge.arrow.size=.5)
legend( "topleft",levels(factor(V(g)$major)),pch=21,col="#777777",pt.bg=color_by_major)
...
library(stringr)
library(igraph)
real_data<-read.csv("hudk4050-classes.csv",T)
names(real_data)=as.character(real_data[1,])

real_data<-real_data[-c(1:2),]

real_data<-tidyr::unite(real_data,"student_name", `First Name`, `Last Name`,sep =" ")

real_data$student_name<-str_replace(real_data$student_name,"`","")
real_data$student_name<-str_to_title(real_data$student_name)

real_data<-real_data %>% mutate_at(2:7,list(toupper))
real_data<-real_data %>% mutate_at(2:7,str_replace_all," ","")

stu_df<-real_data%>%gather(label,class,2:7,na.rm=T,convert = F) %>% select(student_name,class)



stu_df$count <- 1
#remove blank classes
stu_df<-filter(stu_df,class!="")

#remove duplicate classes
stu_df<-unique(stu_df)

stu_df<-spread(stu_df,class,count)

rownames(stu_df)<-stu_df$student_name
stu_df<-select(stu_df,-student_name,-HUDK4050)


#shortest
stu_df[is.na(stu_df)]<-0
head(stu_df,5)
#cheat way:
#stu_df = ifelse(is.na(stu_df),0,1)

#Convert to matrix

stu_df_matrix <- as.matrix(stu_df)


#Person-to-Person matrix

stu_df_matrix<-stu_df_matrix %*% t(stu_df_matrix)


#graphing

my_graph<-graph.adjacency(stu_df_matrix,mode = "undirected",diag=F)
plot(my_graph,layout=layout.fruchterman.reingold,vertex.size=4,
...
#Centrality
sort(betweenness(my_graph),decreasing = T)

#betweeness
sort(degree(my_graph),decreasing = T)
...
### From the graph and result, I can know Yifei Zhang has the biggest betweenness and Yifei is the second place of degree centrality. Thus, Yifei Zhang is the most central person in the network.
...
temp_df = real_data[order(real_data$student_name),]
interestings = temp_df[!duplicated(temp_df$student_name),]$'Which of these topics is most interesting to you?'
V(my_graph)$color=factor(interestings)
plot(my_graph,layout=layout.fruchterman.reingold, edge.width=0.1,vertex.label.cex=0.6)
