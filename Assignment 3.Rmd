...
Yifei Zhang is the most central person in the network according to these two metrics. She lies between two major groups'students, which means there exist a shortest path from all students to all others that pass through her. There are several students have the highest degree at the same time which means they have the highest number of links to other students, however, they are not necessary to be the betweenness centrality.

```{r}
#Calculate the degree centrality of nodes, showing who has the most connections
...
There are three major interest group clusters which are social network analysis, prediction modeling and visualization.
```{r}
C5 <- C1
#Remove unwanted rows
C5 <- slice(C5, 3:49)
