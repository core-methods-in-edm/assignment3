...
K1 <- read.csv('HUDK405018-cluster.csv',na.strings = c("","NA"))
...
```{r}
names(K3) <- lapply(K3[1, ], as.character)
K3 <- K3[-1, ]
K3 <- data.matrix(K3)
K3 <- scale(K3)
```
...
names(K4) <- c("1","2","3","4","5","6","7","cluster")
...
K5 <- gather(K4,'week','motivation',1:7)
```
...
names(Data4) <- c("1","2","3","4","5","6","7","cluster")
Data5 <- gather(Data4,'week','motivation',1:7)
...
DF <- data.frame(D5,D4)
DF$fit4.cluster <- as.factor(DF$fit4.cluster)
DF$fit3.cluster <- as.factor(DF$fit3.cluster)
ggplot(DF,aes(lat,lon,color=fit3.cluster))+geom_point()+geom_jitter(width = 2.5, height = 10)
ggplot(DF, aes(x = lat, y = lon, color = fit3.cluster)) + geom_point()


names(D5) <- c("1","2","3","4","5","6","7","cluster")
D6 <- gather(D5,'week','motivation',1:7)
D7 <- D6 %>% group_by(week,cluster)
D7 <- summarise(D7,avg=mean(motivation))
D7$week <- as.numeric(D7$week)
D7$cluster <- as.factor(D7$cluster)
ggplot(D7,mapping = aes(x=week,y=avg,color=cluster))+geom_line()+xlab("Week")+ylab("Average Motivation")
```
