...
person_class <- select(mydf, 1:8) %>% unite("name","First Name", "Last Name", sep = " ")
person_class$name <- str_to_title(person_class$name)
person_class$name <- str_replace(person_class$name, "`","")
person_class <- mutate_at(person_class,2:7,list(toupper))
person_class <- mutate_at(person_class,2:7,str_replace_all," ","")
person_class[person_class == ""] <- NA
```
