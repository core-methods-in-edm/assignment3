...
ggplot(data = K6) + geom_line(mapping = aes(week, avg, colour = cluster)) + xlab("Week") + ylab("Average Motivation")
...
* Dealing with missing values - there are two problems created by students skipping questions in the survey
1. There are missing values
2. read.csv() is treating those missing values as characters when it imports the data. That means the variables are being stored as factor type and not numeric type and if you try to recode the missing variables it will give an error that you cannot add levels to the factor

* First convert the blank cells to NAs and convert all columns to numeric
```{r}
D2[D2==""] = NA
D2 = D2 %>% mutate_all(funs(as.numeric(levels(.))[.]))
```

* __Solution 1__: Remove students with missing values - but we will lose those students and be unable to allocate them to a group
```{r}
D2 <- na.omit(D2)
```

* __Solution 2__: Recode as zero - but missing is not the same as recording zero hours
```{r}
D2[is.na(D2)] <- 0
```

* __Solution 3__: Recode as some other number that doesn't appear in your data (EG - 99) and rescale
```{r}
D2[is.na(D2)] <- 99
