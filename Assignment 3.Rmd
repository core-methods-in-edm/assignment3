...
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, vertex.size=num*1.5,edge.arrow.size=0.3,margin=-0.1)
```
...
b1=1:2
dnew=d1[-b1,]#delete the first 2 rows
...
b2=c(2:3,length(dnew))
dnew=dnew[,-b2] #delete the last question and first and last names
dnew2=dnew %>% pivot_longer(-fullname,names_to='class',values_to='classname')
dnew2=na.omit(dnew2)
dnew2=dnew2[,-2]
persontoclass=as.matrix(table(dnew2$fullname,dnew2$classname)) #person-class matrix
...
```{r}
int=d1$`Which of these topics is most interesting to you?`[3:49]
clusterpp=cluster_fast_greedy(g2)
plot(clusterpp,g2,layout=layout.fruchterman.reingold, edge.width=0.3,vertex.label.cex=0.3,vertex.color=int)
clusterpc=cluster_fast_greedy(g3)
plot(clusterpc,g3,layout=layout.fruchterman.reingold, edge.width=0.3,margin=-0.2,vertex.label.cex=0.5,vertex.color=int)
```

#Answer: As is seen in these two pictures, nearly all the clusters of interest by different colors correpond to the clustes in the network identified and colored by the RStudio.
