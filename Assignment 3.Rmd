...
plot(g, layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, edge.width=EDGE$count, margin=-0.1, edge.arrow.size=0.3, edge.arrow.width = 0.7, vertex.size =(num)*2,vertex.label.color="black")
```
...
```{r}
#The most degree central person in the network is Guoliang Xu, Hangshi Jin, and those who have 31 as the degree centrality.Yifei Zhang is the most betweenness central person.And Yifei Zhang is the most central person in the network according to these two metrics,shown by both the relatively high degree centrality measure of 24 and betweenness centrality measure of 260.6143603. 

#interpretation:
#Degree centrality is number of links to other nodes in the network,and the degree centrality measure of 24 means Yifei Zhang is connected to 24 other people.

#Betweenness centrality is the extent to which a node lies between other nodes,and the highest betweenness centrality measure (260.6143603) shows that Yifei Zhang is the node through which the highest number of shortest paths pass from all nodes to all others.
```


```{r}
#Color according to interest 
C5<-C1
colnames(C5) <- C5[1,]
C5 <- slice(C5,3:49)
...
C6$interest <- as.factor(C6$interest)
plot(g,layout=layout.fruchterman.reingold,vertex.size = 10,edge.width=0.3,vertex.label.cex=0.3, vertex.color=C6$interest)

#There is not any cluster of interest that correspond to cluster in the network. 
#interpetation
#As can be seen from the graph, many of the students forming the connection have different colored nodes or vertices, indicating Student clusters in the network have different interests, that is, no corresponding interest clusters have been formed.
