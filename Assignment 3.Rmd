...
gather(K42, week, mtv, 1:5)
 group_by(K42, cluster, week) %>% 
	  summarise(avgmtv = mean(mtvvalue)) %>% 
	  arrange(avgmtv)
K42 <- as.factor(K42)
K42$week <- as.numeric(K42$week)
ggplot(K42, aes(week, avgmtv, color = cluster)) + geom_line()
...
```{r}
Data2 <- read.csv("HUDK405018-cluster.csv",skip = 2)
head <- read.csv("HUDK405018-cluster.csv")
head <- colnames(head)
colnames(Data2) <- head
D1 <- select(Data2, LocationLatitude ,LocationLongitude,Q2_1:Q2_7)
D2 <- na.omit(D1)
D3 <- scale(D2[,-c(1:2)])
f <- kmeans(D3, 2)
D4 <- data.frame(D2, f$cluster)
names(D4)[10] <- "cluster" 
D4$cluster <- as.factor(K4$cluster)
```
...
```{r}
ggplot(D4, aes(LocationLatitude, LocationLongitude, color = cluster)) + geom_jitter()
```
