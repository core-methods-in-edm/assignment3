...
```{r}
comments.received <- V.TO %>% count(id) %>% select(id, n)
VERTEX <- VERTEX %>% left_join(comments.received, by = "id") %>% mutate(comments.received = ifelse(is.na(n),0,n)) %>% select(-n)
```

```{r}
pal <- brewer.pal(4,"Accent")
Group <- as.factor(VERTEX$major)
vertex.col <- pal[Group]
plot(g, layout = layout.fruchterman.reingold, vertex.size = 3*(VERTEX$comments.received),vertex.color = vertex.col, edge.arrow.size = .25, vertex.label.color="black",vertex.label.cex=0.75,edge.width=EDGE$count)
...
```{r}
edge_list <- read.csv("hudk4050-classes.csv",skip = 1,header = TRUE,strip.white = TRUE)

#remove first row of unwanted data
edge_list <- edge_list[-1,]

#make all classes uppercase
edge_list[3:8] <- map(edge_list[3:8],str_to_upper)
#remove any spaces in classes
edge_list[3:8] <- map(edge_list[3:8],str_replace_all," ","")

#rename the last column to a shorter name, topic
names(edge_list)[ncol(edge_list)] <- "Topic"
#Combine first and last name into one column, Full.Name
edge_list$Full.Name = paste(edge_list$First.Name,edge_list$Last.Name)
edge_list <- edge_list %>% select(-First.Name,-Last.Name) %>% select(Full.Name,everything())
#remove topic for matrix processing
edge_list_no_topic <- edge_list %>% select(-Topic)

#create person_course matrix using gather, unique, then spread
person_course_mat <- gather(edge_list_no_topic,key = "Class", value = "Course",2:7) %>% arrange(Full.Name) %>% filter(Course != "") %>% select(-Class) %>% unique %>% mutate(number = 1) %>% spread(Course,number) %>% mutate(HUDK4050 = 1)

#remove HUDK4050 because everyone listed it as a class, and it makes the graph difficult to read
person_course_mat <- subset(person_course_mat, select=-HUDK4050)

#replace all NAs with 0's
person_course_mat[is.na(person_course_mat)] <- 0

#replace rownames with Full.Name column
rownames(person_course_mat) <- person_course_mat[,1]
person_course_mat <- person_course_mat[,-1]
person_course_mat <- as.matrix(person_course_mat)

#create a course_person matrix which is the transpose of the person_course matrix
course_person_mat <- t(person_course_mat)

#multiple the two matrices to compute the person_person matrix
person_person_mat <- person_course_mat %*% course_person_mat

g <- graph_from_adjacency_matrix(person_person_mat,weighted = TRUE,diag = FALSE,mode = "undirected")
...
The most connected person based on betweenness centrality was `r g %>% betweenness %>% sort(decreasing = TRUE) %>% names %>% .[[1]]`. The person with the highest degree was `r g %>% degree %>% sort(decreasing = TRUE) %>% names %>% .[[1]]`

Betweenness centrality identifies how influential people are at connecting other people. Thus, Yifei may be taking one or two courses that only a few other students take who would otherwise not be connected to the central component.

Degree centrality identifies the number of connections each person has. Yifei also happened to have the highest degree which means he also was in a few classes that many other students were taking.
...
Group <- as.factor(edge_list$Topic)
vertex.col <- pal[Group]
plot(g,vertex.label = NA,vertex.size = 10, vertex.color = vertex.col)
legend("topleft",bty = "n",
       legend=levels(Group),
       fill=pal, border=NA)
```

Once all of the connections from HUDK4050 were removed, interestingly, there were no noticeable patterns in the network based on interest. However, all students interested in data wrangling were part of the large connected component. Data wrangling is a core component of HUDK4050 and thus students may take other data-based classes similar to their peers in the class.
