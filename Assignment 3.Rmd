...
K1 <-data.frame(read.csv("Class_Motivation.csv", header = T))
...
```{r}
ggplot(data=K6[, c(1,3)], aes(x=K6$week,y=K6$mean, color=K6$cluster)) +
   geom_line(data=K6[, c(1,3)],aes(x=K6$week, y=K6$mean, color=K6$cluster,size=2))+
...
```{r}
D1<-data.frame(read.csv("HUDK405018-cluster.csv", header = T, na.strings = ""))
D1<-D1[3:47, ]
D2<-D1[ ,20:26]
D3<-na.omit(D2)
...
names(D4)<-c("1","2","3","4","5","6","7","cluster")

D5<-gather(D4,"activity","hours",1:7)
D6<-D5%>%group_by(activity,cluster)%>%summarize(avg_hours=round(mean(as.numeric(hours))))
D6$cluster<-as.factor(D6$cluster)
D6$activity<-as.numeric(D6$activity)

ggplot(D6,aes(x=activity,y=avg_hours,col=cluster))+ geom_line()+xlab("Activity")+ylab("Average Hours")

library(ggmap)
D7<- geocode(as.character(D3$Q2_1), output="latlon", source="dsk")
D8<-scale(D7)
regions<-kmeans(D8,2)
regions$cluster
D9<-data.frame(D8,regions$cluster)
D9$regions.cluster<-as.factor(D9$regions.cluster)
ggplot(data=D9, aes(lon,lat,color=regions.cluster))+geom_point(data=D9, aes(lon,lat,color=regions.cluster),size=4)+xlab("Longitude")+ylab("Latitude")
