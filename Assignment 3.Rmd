...
library(tidyr)
library(stringr)
plot(g, layout = layout.fruchterman.reingold, vertex.color = VERTEX$major, edge.arrow.size = 0.4, edge.arrow.width = 0.6, edge.width = EDGE$count, margin = -0.1, vertex.size =  (EDGE$count*4), vertex.label.color = "black")
...
# Yifei Zhang is the most central person in the network as shown by these 2 values. They have a relatively high degree centrality of 24, meaning they are connected to 24 people and betweenness centrality of 260.6143603, meaning they are the node that connect the highest number of shortest paths between all nodes.
C5 <- C1
colnames(C5) <- C5[1, ]
C5 <- slice(C5, 3:49)
...
# The plot shows there is no cluster of interest that correspond to the cluster in the network since students forming connection has different colors, thus different interests.
```
