...
```{r}
plot(g,layout=layout.fruchterman.reingold, vertex.color=VERTEX$major, vertex.size = (EDGE$count*10), edge.width=1, margin=-0.1, edge.arrow.size=.5, edge.arrow.width=0.8)
```
...
library(stringr)
library(igraph)
class_df <- read_csv("hudk4050-classes.csv", skip = 1)
class_df
class_df <- class_df[-1,]
names(class_df) = c("FirstName","LastName","Class1","Class2","Class3","Class4","Class5","Class6","Interest")
class_df = unite(class_df, "Name", `FirstName`, `LastName`, sep = " ")
class_df$Name = str_replace(class_df$Name, "`", "")
class_df$Name = str_to_title(class_df$Name)
class_df = class_df %>% mutate_at(2:7, str_replace_all, " ", "")
class_df1 = select(class_df, 1:7)
class_df1
```
...
```{r}
library(tidyverse)
class_df2 <- class_df1 %>% gather(label, Class, 2:7, na.rm = TRUE, convert = FALSE ) %>% select(Name,Class)
class_df2$Count <- 1
class_df2 <- filter(class_df2, Class != "")
class_df2 <- unique(class_df2)
class_df2 <- spread(class_df2,Class,Count)
class_df2 <- class_df2 %>% column_to_rownames(var="Name")
class_df2 <- select(class_df2,-HUDK4050)
class_df2 [is.na(class_df2)]<- 0
class_df2
```

```{r}
class_df3 = as.matrix(class_df2)
class_df3
connection_matrix <- class_df3 %*% t(class_df3)
```
```{r}
g <- graph.adjacency(connection_matrix, mode="undirected", diag = FALSE)
plot(g, layout=layout.fruchterman.reingold,
     vertex.size= 4,
...
```
The most connected person based on betweenness centrality was Yifei Zhang. The person with highest-degree is Guoliang Xu.
This means that Guoliang Xu knew the most students and Yifei Zhang was the most influential person when connecting with other students. 
...
Interest <- as.factor(class_df$Interest)
Interest
plot(g, layout=layout.fruchterman.reingold,
     vertex.size= 4,
     vertex.label.cex=0.8,
...
legend("topleft",bty = "n",
       legend=levels(Interest),
       border=NA, fill=brewer.pal(5,"Accent"))
```
Not identifiable pattern in this group. Seems like Data Wrangling is common among all the students.
