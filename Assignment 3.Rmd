...
```{r}
D2 <- select(D1, comment.from, comment.to)
EDGE <- count(D2, comment.from, comment.to)
attach(EDGE)
EDGE1 <- EDGE[order(comment.from),]
...
EDGE1 is your edge list. Now we need to make the vertex list, a list of all the students and their characteristics in our network. Because there are some students who only recieve comments and do not give any we will need to combine the comment.from and comment.to variables to produce a complete list.

```{r}

#First we will separate the commenters from our commentees
V.FROM <- dplyr::select(D1, comment.from, from.gender, from.major)

#Now we will separate the commentees from our commenters
V.TO <- dplyr::select(D1, comment.to, to.gender, to.major)
...
VERTEX <- dplyr::full_join(mutate(V.FROM, id=factor(id, levels=lvls)),
    dplyr::mutate(V.TO, id=factor(id, levels=lvls)), by = "id")
...
VERTEX <- dplyr::select(VERTEX, id, gender.from, major.from)
...
Now we have both a Vertex and Edge1 list it is time to plot our graph!
...
```{r}
EDGE1$sumcount <- with(EDGE1, ave(count, to,FUN = sum))
EDGE2 <- select(EDGE1, to, count, sumcount)

names(EDGE2) <- c("id", "count_to","totalcommentreceived")


EDGE1$sumcount <- with(EDGE1,ave(count, to,FUN = sum))
EDGE2 <- select(EDGE1, to, count, sumcount)

names(EDGE2) <- c("id", "count","totalcommentreceived")


EDGE2 <- left_join(VERTEX,EDGE2,by=c("id"))
EDGE2$totalcommentreceived[is.na(EDGE2$totalcommentreceived)] <- 0

plot(g,layout=layout.fruchterman.reingold, directed=TRUE, edge.arrow.size=0.4, vertex.size=5+EDGE2$totalcommentreceived, vertex.color=VERTEX$major)
```
...
#import data
```{r}
C1 <- read.csv("hudk4050-classes.csv", stringsAsFactors = FALSE, header=TRUE, na.strings=c("","NA"))
...
C2 <- slice(C2, 3:49)
C2 <- select(C2, 1:8)
C2 <- tidyr::unite(C2, "name", `First Name`, `Last Name`, sep=" ")

C2$name <- str_replace(C2$name, "`", "")
C2$name <- str_to_title(C2$name)
C2 <- C2 %>% mutate_at(2:7,list(toupper))
C2 <- C2 %>% mutate_at(2:7, str_replace, " ", "")
```

#data restructuring
```{r}
C3 <- C2 %>% tidyr::gather (label, class, 2:7, na.rm=T, convert=F) %>%
  select(name, class)
...
C4 <- tidyr::spread (C3, class, count)

rownames(C4) <- C4$name

C4 <- select(C4, -name, -HUDK4050)
#alternative: C4 <- C4[ -c(1,...) ]
...
##graphing
```{r}
g <- graph.adjacency(C6, mode="undirected", diag = FALSE)
plot(g, layout=layout.fruchterman.reingold, vertex.size=4, vertex.label.cex=0.8, vertext.label.color="blue",vertex.color="gainsboro")
```
