...
class_df$Name = paste(class_df$FirstName,class_df$LastName)
class_df
class_df$Name <- str_replace(class_df$Name,"`","")
class_df$Name <- str_to_title(class_df$Name)
class_df1 <- class_df %>% gather(label, Class, 2:7, na.rm = TRUE, convert = FALSE ) %>% select(Name,Class)
```

```{r}

class_df1$Count <- 1
class_df1 <- filter(class_df1, Class != "")
class_df1 <- unique(class_df1)
class_df1 <- spread(class_df1,Class,Count)
rownames(class_df1) <- class_df1$Name
class_df1 <- select(class_df1,-Name,-HUDK4050)
class_df1 [is.na(class_df1)]<- 0
```
```{r}

connection_matrix <- as.matrix(class_df1) %*% t(as.matrix(class_df1))
```

Once you have done this, also [look up](http://igraph.org/r/) how to generate the following network metrics:

```{r}
pt <- graph_from_adjacency_matrix(connection_matrix, mode="undirected", diag = FALSE)
