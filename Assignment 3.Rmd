...
fit_hour <- kmodes(H3, 4, iter.max = 10)
...
fit_location <- kmodes(H7, 3, iter.max = 10)
...
```{r}
library(ggplot2)
##ggplot(H6, mapping, aes = )

ggplot(H8, mapping = aes(Question_number, hours, color = hour_cluster, shape = location_cluster)) +
  geom_point() +
  xlab("Question#") + ylab("hours")

##This cannot run because of "Error: A continuous variable can not be mapped to shape"
##Also, not sure to use plot() or ggplot() here.
##Sorry my logic is really messy over here. Will try to fix it later.
```
