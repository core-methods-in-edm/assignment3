...
K0 <- read.csv("class_motivation.csv")
K1 <- data.frame(K0)
...
# Recoding the values will impact the how the data is calculated and if we delete values and it will impact the clusters and mean especially since it is a small data set
...
install.packages("ggplot2")
library(ggplot2)
ggplot(K6, aes(week, avg, colour = cluster)) + geom_line() + xlab("Week") + ylab("Motivation over 5 Weeks")

```

What patterns do you see in the plot?
# cluster 1 kids start off with little motivation but increase by week 3. Opposite for cluster 2 kids. 
