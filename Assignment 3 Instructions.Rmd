...
/*********ANSWER***********/
If we remove them, then it might affect none "NA" variables summary and the calculation of those variable might be unaccurate.
If we treat them as having meaning, then this "NA" values might affect the summary of the variable they are in. For example the mean could be different, the variance could be difference and so on.
/**************************/
...
```{r}
fit3 <- kmeans(K3, 3) #create cluster
fit3$cluster #checkout clusters
K4_3 <- data.frame(K3, fit3$cluster) #merge clusters into the table
names(K4_3) <- c("1", "2", "3", "4", "5", "cluster")  #changing names
K5_3 <- tidyr::gather(K4_3, "week", "motivation", 1:5) 
  #combine all motivations into one colume and add a "week" colume to mark motivations
K6_3 <- K5_3 %>% group_by(week, cluster) #group data by Week and Clusters
K6_3 <- summarise(K6_3, avg = mean(motivation)) #average motivations based on week and clusters
K6_3$week <- as.numeric(K6_3$week) #change week from factor to numeric
K6_3$cluster <- as.factor(K6_3$cluster) #change cluster into factor
ggplot(K6_3, aes(week, avg, colour = cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
  #Run ggplot to graph
K7_3 <- dplyr::count(K4_3, cluster) #count members in each cluster
K7_3 #show up the result
```
...
```{r}
Class_data <- read.csv("cluster-class-data.csv")
names(Class_data) <- c("Duration","FName","LName","OwnCat","gif","Mon_in_NY","siblings","t_sport","miles","Android","movies","classes","state_v","city_g","state_g","country_g")
Class_data
Class_data_choose <- dplyr::select(Class_data, 10:13,6)
Class_data_choose <- scale(Class_data_choose)
fit3 <- kmeans(Class_data_choose, 2) #create cluster
fit3$cluster #checkout clusters
K4_3 <- data.frame(Class_data_choose, fit3$cluster) #merge clusters into the table
K4_3
names(K4_3) <- c("1", "2", "3", "4", "5", "cluster")  #changing names
K5_3 <- tidyr::gather(K4_3, "Question", "Answers", 1:5) 
  #combine all motivations into one colume and add a "week" colume to mark motivations
K6_3 <- K5_3 %>% group_by(Question, cluster) #group data by Week and Clusters
K6_3 <- summarise(K6_3, avg = mean(Answers)) #average motivations based on week and clusters
K6_3$Question <- as.numeric(K6_3$Question) #change week from factor to numeric
K6_3$cluster <- as.factor(K6_3$cluster) #change cluster into factor
ggplot(K6_3, aes(Question, avg, colour = cluster)) + geom_line() + xlab("Question") + ylab("Average Answers")
  #Run ggplot to graph
K7_3 <- dplyr::count(K4_3, cluster) #count members in each cluster
K7_3 #show up the result
```
