...
1. If we remove them, then we may lose the information for those students from the dates when they were present
2. If there's a particular reason why people were missing on some days but not others (e.g. the very advanced students were at a job fair) then it may create a systematic bias in our data
...
-At week 1, Cluster 1 was the most motivated and Cluster 2 was the least motivated. By Week 5, they had flipped, with Cluster 2 more motivated than Cluster 1.
-The trend partially reversed itself in Week 3, when Cluster 1 again became more motivated and Cluster 2 again became less motivated.
...
K4b <- data.frame(K3, fit3$cluster)

#Changing the names of the variables in K4b to make it more convenient with the names() command.
...
#Averaging by week and cluster
K6b <- K5b %>% group_by(week, cluster)
K6b <- summarise(K6b, avg = mean(motivation))

#Changing the format of the data into something that can be plotted
