...
loc3 <- mutate_geocode(loc3, location) # did confirm that "Hanoi, NA, Vietnam" returned the correct coordinates, even with its missing state/province data
...
* Cluster location data using k-means, then create new data frame with appended cluster data
```{r}

fitloc <- kmeans(loc4, 4)
loc5 <- data.frame(loc4, fitloc$cluster)
names(loc5) <- c("lon", "lat", "cluster")
