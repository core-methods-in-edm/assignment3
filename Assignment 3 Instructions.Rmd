...
#rename columns so I don't drive myself crazy
names(CD) <- c("time", "First", "Last", "Cat", "Gif", "NYC Months", "Siblings", "Sports", "Miles", "Android", "Movies", "Classes", "States", "City", "State", "Country") 

#change cat y/n and gif jif to binary (create B versions of the column with numbers)
CD$CatB <- 0
CD$CatB[CD$Cat == "Yes"] <- 1
CD$GifB <- 0
CD$GifB[CD$Gif == "g-iff"] <- 1

#make a numeric column for locations. distance in miles from NYC (obvi). use Wellesley MA not Vietnam...because had to make a choice. 
```