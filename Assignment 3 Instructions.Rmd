...
K1 <- read.csv("C:/Users/Josh/Documents/R/Assignment 3/Class_Motivation.csv")
```
...
Removing values may skew the results by excluding important data points. Recoding the values may change the scale and cluster formations. 
...
I'm guessing the one with three clusters because the range between the two clusters is extreme. 

---
title: "Assignment 3: K Means Clustering with 3 groups"
---
```{r}
fit <- kmeans(K3, 3) 
fit$cluster
...
# I played around with trying to clean the data for clustering. I would definitely like to review this to learn how to do this with R scripting instead of excel... 
...
C1 <- read.csv("cluster-class-data.csv", header = TRUE)
C2 <- dplyr::select(C1, 2:16)

names(C2)
names(C2)<- c("First", "Last","catlife","gif","monthsNY","siblings","sports","miles","Friend_Android","movies","classes","Vstates","city","State_Province","country")
C3 <- na.omit(C2) 
#Plot by location and lastname 
ggplot(C3, aes(State_Province, country, label = Last)) + geom_point()
RegC3 <- dplyr::select(C3, 13:15)

set.seed(10992)
C4 <- kmeans(C3[,c("monthsNY","classes")], 3, nstart=10)
c4cluster = factor(C4$cluster, labels = c("monthsNY"))

```


```{r}
#Clustering by class answers to questions 

CQonly <- dplyr::select(C3, 5:11)
View(CQonly)
CQResults <- kmeans(CQonly, 3)
CQResults
CQResults$size
 table(C3$country, CQResults$cluster)
 
plot(CQonly[c("monthsNY","classes")], col = CQResults$cluster)
 
plot(CQonly[c("monthsNY","Friend_Android")], col = CQResults$cluster)

plot(CQonly[c("classes","movies")], col = CQResults$cluster)

ggplot(CQonly, aes(monthsNY, classes, colour = cluster)) + geom_line() + xlab("months") + ylab("classes")