...
K1 <- read.table("C:/Users/Nino Zhang/Documents/Nino Zhang/study at TC/fall 2016/EDM/assig5/assignment on 10/20/Class_Motivation.csv", header=TRUE, 
  	sep=",")
...
A: Missing values might heavily effect the whole data set. They might contain crucial meaning when come to clustering. Say, if we want to group the data based on the absense of response, then if the value is missing will become an essential condition for clustering
...
```{r}
# we can change the number of clusters into 3
fit2 <- kmeans(K3, 3)
B4 = data.frame(K3, fit2$cluster)
names(B4) <- c("1", "2", "3", "4", "5", "cluster")
B5 <- tidyr::gather(B4, "week", "motivation", 1:5)
B6 <- B5 %>% group_by(week, cluster)
B6 <- summarise(B6, avg = mean(motivation))
B6$week <- as.numeric(B6$week)
B6$cluster <- as.factor(B6$cluster)
ggplot(B6, aes(week, avg, colour = cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
```
...
D1 <- read.table("C:/Users/Nino Zhang/Documents/Nino Zhang/study at TC/fall 2016/EDM/assig5/assignment on 10/20/cluster-class-data.csv", header=TRUE, 
  	sep=",")
...
```{r}
#D2 <- dplyr::select(D1, 4:16)
# merge columns the assign names to each of them
D2 <- tidyr::unite(D1, QID1.2, QID2_TEXT, QID1_TEXT, sep = ", ", remove = TRUE)
names(D2) <- c("duration", "name", "cat_or_not?", "j_or_g?", "time_in_NY", "siblings", "sports", "dist_to_TC", "phone", "movie", "classes", "state", "town", "prov", "country") 
...
```{r}
# then we have to convert responses in yes/no into binary numbers.
D2$`cats or not?` <- ifelse(D2$`cat_or_not?`=="Yes", 1, 0)
...
```{r}
D3 = dplyr::select(D2, 9:12, 5)
D3 = scale(D3)
fit <- kmeans(D3, 4)
# we combine the clusters into a table
D4 = data.frame(D3, fit$cluster)
# then we assign names
names(D4) = c("1", "2", "3", "4", "5", "cluster")
D5 <- tidyr::gather(D4, "questions", "responses", 1:5)
D6 <- D5 %>% group_by(questions, cluster)
D6 <- summarise(D6, avg = mean(responses))
D6$questions = as.numeric(D6$questions)
D6$cluster = as.factor(D6$cluster)
ggplot(D6, aes(questions, avg, colour = cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
